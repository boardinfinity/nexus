...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // ==================== PLACEINTEL ADMIN ROUTES (behind main auth) ====================
...
<p>Thank you,<br/>Board Infinity Team</p>`,
    ...
    if (path === "/placeintel/sync-board-hub" && req.method === "POST") {
      ...
      const bhRes = await fetch(`${boardHubUrl}?account_type=eq.Higher-ed&select=id,name,city,state,priority`, {
        headers: { apikey: boardHubKey, Authorization: `Bearer ${boardHubKey}` },
      });
      if (!bhRes.ok) return res.status(500).json({ error: `Board Hub API error: ${bhRes.status}` });
      const accounts: any[] = await bhRes.json();

      let new_count = 0, updated_count = 0, skipped_count = 0;

      for (const acc of accounts) {
        // Check if already linked
        const { data: existing } = await supabase.from("colleges").select("id").eq("board_hub_account_id", acc.id).single();
        if (existing) {
          skipped_count++;
          continue;
        }
        // Check if college exists by name (for existing catalog colleges)
        const { data: byName } = await supabase.from("colleges").select("id, board_hub_account_id").ilike("name", acc.name).single();
        if (byName && !byName.board_hub_account_id) {
          await supabase.from("colleges").update({
            board_hub_account_id: acc.id,
            state: acc.state,
            priority: acc.priority,
            account_type: "Higher-ed",
          }).eq("id", byName.id);
          updated_count++;
        } else if (!byName) {
...
async function handlePlaceIntelRoutes(path: string, req: VercelRequest, res: VercelResponse): Promise<VercelResponse> {
