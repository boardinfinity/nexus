...
| 043 | `043_upsert_skill_external_id.sql` | 2026-05-15 | camjdbcab | Fix `upsert_skill` (both 3-arg and 5-arg overloads) — supply `external_id` and `source` on insert. `taxonomy_skills.external_id` is NOT NULL with no default, so the previous versions failed with 23502 on every auto-skill creation, silently breaking analyze-jd skill extraction across JD Analyzer + Campus Upload. New `external_id = 'nx-auto-' || substr(sha256(name_normalized), 1, 10)` (qualified `extensions.digest`), `source = 'auto'`. Uses `ON CONFLICT (external_id, source) DO NOTHING` + re-SELECT for concurrent races. Keeps SECURITY DEFINER + pinned search_path + REVOKE FROM PUBLIC + GRANT to service_role only (Phase 2 baseline). Smoke-tested idempotent on jlgstbucwawuntatrgvy. Advisors clean (0 ERROR / 1 WARN baseline). Commit f0ebb2b. |
...
| 043 | _(applied — see main table)_ | camjdbcab | applied 2026-05-15 |
| 044 | `044_campus_excel_support.sql` | 2026-05-15 | camjdbcab | Campus Excel Upload support. New tables: `campus_vacancies` (Track B title-only vacancy log output) + `campus_excel_tasks` (Track A async work queue for full-JD Excels). Adds `campus_upload_batches.batch_type` column with CHECK ('jd_analyze','excel_jd_analyze','vacancy_log'). Both new tables RLS-enabled, service-role-only (no policies). Applied 2026-05-15. Advisors: 0 ERROR / 1 WARN baseline. |
| 045 | _(open)_ | TBD | available |
