...
- 2026-05-07 · track-E-runs-dashboard · 18ef398 · Track E: /jd-analyzer/runs dashboard page — KPI strip (total runs, success/partial/failure rates, avg latency, skills extracted/new), filter bar (source/status/date), runs table with status badges + job links + bucket info + truncated errors, side-panel drill-down with full error message + Re-run button (POST /api/analyze-jd with job_id) + auto-refresh toggle (10s). Sidebar link 'Analyzer Runs' added under JD Analyzer. Empty state messages migration 038 + filter-zero. Admin-only v2 (TODO for SPOC scoping). Commits: 18ef398 (jd-analyzer-runs.tsx), 3281c6e (App.tsx), 82a46f5 (app-sidebar.tsx).
